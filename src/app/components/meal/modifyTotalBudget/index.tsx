...
function ModifyTotalBudget({ newTotalBudget, close, opened, selectedRows }: any) {
  ...
  const { mutate } = useMutation({
    mutationFn: (values: any) => postApi.updateMealBudgetByPerson(values),
  });

  const modifyNote = async () => {
    mutate(
      { body: { mealBudget: newTotalBudget }, params: selectedRows.mealStatsIdx },
      {
        onError: (error) => {
          notification({
            color: "red",
            title: "식대 총 금액 수정",
            message: "식대 총 금액 수정에 실패하였습니다.",
          });
        },

...
export const Welfares = memo(({ data, handleModifyNote, setNewTotalBudget, setSelectedRows, selectedRows, selectedSubRows, setSelectedSubRows, openModifyNote }: any) => {
        ...
        <Table.Tr className="hover:bg-gray-100">
          ...
          <Table.Td onClick={() => hasDetails && toggleRow(element.welfareIdx)} style={{ cursor: hasDetails ? "pointer" : "default" }}>
            <Group gap="xs">
              <Box w={20}>
                {hasDetails &&
                  (expandedRows.includes(element.welfareIdx) ? <IconChevronDown color="var(--mantine-color-gray-5)" size={16} /> : <IconChevronRight color="var(--mantine-color-gray-5)" size={16} />)}
              </Box>
              ...
              <Collapse in={expandedRows.includes(element.welfareIdx)}>
                <Box px={"xl"} py={"xs"} pb={"lg"} bg="gray.0" style={{ borderBottomRightRadius: 12, borderBottomLeftRadius: 12 }}>
                    ...
                    <Table.Tbody>
                      {element.details.payeeList.map((payee: TWelfares) => (
                        <Table.Tr key={payee.welfareIdx}>
                          <Table.Td>
                            <Checkbox size="xs" checked={subRowsSelected.includes(payee.welfareIdx)} onChange={() => toggleSubRowSelection(element.welfareIdx, payee.welfareIdx)} />
                          </Table.Td>
                  ...
                  <Group gap={"xs"} align="center" mt={"md"}>
                    ...
                    <NumberFormatter style={{ fontSize: "var(--mantine-font-size-sm)", fontWeight: 500 }} thousandSeparator value={element.groupTotalAmount || 0} suffix=" 원" />
                  </Group>
                </Box>
              </Collapse>
            </Table.Td>
